...
/// BPB (BIOS Parameter Block) â€” Boot Sector
#[repr(C, packed)]
...
impl Fat32Fs {
    /// Mount FAT32 filesystem tá»« block device
    ...
    /// Parse directory entries tá»« raw data
...
/// Táº¡o minimal FAT32 image trong memory Ä‘á»ƒ test
pub fn create_test_fat32_image() -> Vec<u8> {
    // 1MB FAT32 image: 2048 sectors Ã— 512 bytes
    let total_sectors: u32 = 256; // 128KB
    let mut img = alloc::vec![0u8; total_sectors as usize * 512];

    let sectors_per_cluster: u8 = 8;  // 4KB clusters
    let reserved_sectors: u16 = 32;
    let num_fats: u8 = 2;
    let fat_size: u32 = 1;  // 4 sectors per FAT (enough for 512 entries)
