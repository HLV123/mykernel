/// Syscall Table â€” Linux x86_64 ABI Compatible
///
/// musl libc cáº§n cÃ¡c syscalls sau Ä‘á»ƒ hoáº¡t Ä‘á»™ng:
///
/// **Process:**
...
/// Dispatch syscall â€” called from usermode.rs syscall_handler
...
// ---------------------------------------------------------------------------
// File descriptor table (global â€” will be per-process in future)
// ---------------------------------------------------------------------------
...
fn sys_write(fd: u64, buf_ptr: u64, len: u64) -> i64 {
    // fd 1 (stdout) and fd 2 (stderr) â†’ VGA + serial
    if fd == 1 || fd == 2 {
...
fn sys_mmap(addr: u64, len: u64, _prot: u64, flags: u64, _fd: i64, _offset: u64) -> i64 {
    // Simple anonymous mmap â€” allocate from a bump allocator region
    if flags & MAP_ANON == 0 {
...
fn sys_fcntl(fd: u64, cmd: u64, _arg: u64) -> i64 {
    match cmd {
        1 => 0,  // F_GETFD â†’ return 0 (no close-on-exec)
        2 => 0,  // F_SETFD â†’ success
        3 => if fd == 0 { O_RDONLY as i64 } else { (O_WRONLY | O_APPEND) as i64 }, // F_GETFL
        4 => 0,  // F_SETFL â†’ success
        _ => EINVAL,
    }
}
...
fn sys_getdents64(fd: u64, buf_ptr: u64, count: u64) -> i64 {
    // Simplified getdents64 â€” not fully implemented
