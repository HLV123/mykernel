...
///
/// 1. BSP (Bootstrap Processor) khá»Ÿi Ä‘á»™ng bÃ¬nh thÆ°á»ng
/// 2. BSP Ä‘á»c ACPI MADT Ä‘á»ƒ tÃ¬m cÃ¡c AP LAPIC IDs
/// 3. BSP ghi trampoline code vÃ o Ä‘á»‹a chá»‰ tháº¥p (0x8000)
/// 4. BSP gá»­i INIT + SIPI IPIs cho má»—i AP
/// 5. AP nháº£y vÃ o trampoline (real mode) â†’ protected mode â†’ long mode
/// 6. AP gá»i ap_main(), set CPU-local state, signal ready
/// 7. BSP chá» táº¥t cáº£ APs ready, rá»“i tiáº¿p tá»¥c
///
/// VÃ¬ QEMU default chá»‰ cÃ³ 1 CPU, Phase 19 sáº½:
/// - Implement Ä‘áº§y Ä‘á»§ infrastructure
/// - Detect sá»‘ CPUs tá»« CPUID / ACPI
/// - Boot APs náº¿u cÃ³ (gracefully skip náº¿u khÃ´ng cÃ³)
...
/// Get number of logical processors from CPUID
pub fn detect_cpu_count() -> u32 {
    // CPUID leaf 0x1: EBX[23:16] = max APIC IDs
    // CPUID leaf 0xB: more accurate topology info
    // For simplicity, use CPUID.1.EBX[23:16]
    let count = unsafe {
        let result: u32;
        core::arch::asm!(
            "cpuid",
    ...
    // count = max APIC IDs (may be 1 for single-core)
    let n = if count == 0 { 1 } else { count };
    crate::serial_println!("[smp] CPUID max APIC IDs: {}", n);
    n
}

/// Get current CPU's APIC ID via CPUID
...
// ---------------------------------------------------------------------------
// ACPI MADT parser â€” find Local APIC entries
// ---------------------------------------------------------------------------
...
fn write_ap_trampoline(phys_mem_offset: u64) {
    ...
    // cli; hlt (2 bytes) â€” minimal safe trampoline
    let trampoline: &[u8] = &[
        0xFA, // cli
        0xF4, // hlt
    ];
