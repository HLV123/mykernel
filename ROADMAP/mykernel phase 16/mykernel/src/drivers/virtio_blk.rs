...
///
/// Implement read/write blocks tá»« virtio-blk device trong QEMU.
///
/// Flow cho má»™t read request:
///   1. Build 3-descriptor chain:
///      [0] Header (type=READ, sector=N) â†’ readable by device
///      [1] Data buffer (512 bytes)      â†’ writable by device
///      [2] Status byte                  â†’ writable by device
...
/// Aligned virtqueue buffers â€” cáº§n align 4096 cho PFN
#[repr(C, align(4096))]
struct VirtqueueBuffers {
    desc:  [VirtqDesc; VIRTQUEUE_SIZE],
    avail: VirtqAvail,
    _pad:  [u8; 4096],  // padding Ä‘á»ƒ used ring align 4096
    used:  VirtqUsed,
}
...
impl VirtioBlkDev {
    /// Khá»Ÿi táº¡o virtio-blk device
    pub fn new(pci_dev: &PciDevice) -> Option<Self> {
        ...
        unsafe {
            ...
            let queue_virt = queue_buf.as_ref() as *const _ as u64;
            // Physical address = virtual - phys_mem_offset (bootloader mapping)
            // phys_mem_offset is stored in process module
            let phys_offset = crate::process::get_phys_mem_offset().as_u64();
            let queue_phys = if queue_virt >= phys_offset {
                queue_virt - phys_offset
            } else {
                queue_virt // fallback
            };
    ...
    /// Äá»c má»™t sector (512 bytes) tá»« disk
    ...
    /// Ghi má»™t sector (512 bytes) ra disk
    ...
    /// Thá»±c hiá»‡n má»™t I/O request qua virtqueue
    fn do_request(&mut self, req_type: u32, sector: u64, buf: &mut [u8; SECTOR_SIZE])
        -> Result<(), &'static str>
    {
        // Build request header vÃ  status buffer trÃªn stack
        ...
        // Poll used ring (busy wait â€” will be replaced with interrupt in future)
...
/// Khá»Ÿi táº¡o virtio-blk driver â€” scan PCI, init device
pub fn init() -> bool {
